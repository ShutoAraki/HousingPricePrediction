...
"""
    ...
    This file is for hyperparameter tuning in GradientBoostingRegressor.
    The computation requires 30 minutes with
...
def model_test_gbr(df, n_jobs):
    ...
    model = model_selection.GridSearchCV(estimator=gbr, param_grid=param_grid, n_jobs=n_jobs, cv=10, scoring=RMSE)
    model.fit(X, y)
...
model_test_gbr(originalDF, n_jobs=44)

"""

def model_test_ridge(df, n_jobs):
    X, y = preprocess(df)
    ...
    model = model_selection.GridSearchCV(estimator=ridge, param_grid=param_grid, n_jobs=n_jobs, cv=10, scoring=RMSE)
    model.fit(X, y)
    print('Gradient boosted tree regression...')
    print('Best Params:')
    print(model.best_params_)
...
def model_test_lasso(df, n_jobs):
    X, y = preprocess(df)
    lasso = Lasso(alpha=0.01, 
                  random_state=0)
    param_grid = {
        'alpha': [i * 0.00001 for i in range(1, 10)]
    }
    model = model_selection.GridSearchCV(estimator=lasso, param_grid=param_grid, n_jobs=n_jobs, cv=10, scoring=RMSE)
    model.fit(X, y)
    print('Gradient boosted tree regression...')
    print('Best Params:')
    print(model.best_params_)
