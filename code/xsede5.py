...
def model_test_rf(df, n_jobs):
    X, y = preprocess(df)
    knn = KNeighborsRegressor()
    param_grid = {
        'n_neighbors': range(2, 15)
    }
    model = model_selection.GridSearchCV(estimator=knn, param_grid=param_grid, n_jobs=n_jobs, cv=10, scoring=RMSE)
    model.fit(X, y)
...
model_test_rf(originalDF, n_jobs=44)
